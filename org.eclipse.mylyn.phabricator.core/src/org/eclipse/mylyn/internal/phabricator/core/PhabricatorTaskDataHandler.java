 ...
 */
public class PhabricatorTaskDataHandler extends AbstractTaskDataHandler {
	...
	public PhabricatorTaskDataHandler(PhabricatorRepositoryConnector connector) {
...
//			return downloadTaskData(repository, PhabricatorRepositoryConnector.getTicketId(taskId), monitor);
//		} finally {
//			monitor.done();
//		}
//	}
...
//					if (!changedAttributes.contains(attribute) && !PhabricatorAttributeMapper.isInternalAttribute(attribute)) {
...
//		if (existingTask) {
//			data.getRoot()
//					.createAttribute(PhabricatorAttributeMapper.NEW_CC)
//					.getMetaData()
//					.setType(TaskAttribute.TYPE_SHORT_TEXT)
//					.setReadOnly(false);
//			data.getRoot().createAttribute(PhabricatorAttributeMapper.REMOVE_CC);
//			data.getRoot()
...
//			Map<String, String> values = PhabricatorAttributeMapper.getRepositoryOptions(client, attr.getId());
...
//	}
	...
	public RepositoryResponse postTaskData(TaskRepository repository, TaskData taskData,
		...
		System.out.println("TracTaskDataHandler.postTaskData()");
...
//		}
	...
	public boolean initializeTaskData(TaskRepository repository, TaskData data, ITaskMapping initializationData,
		...
		System.out.println("TracTaskDataHandler.initializeTaskData()");

		TaskMapper tm = new TaskMapper(data);

		tm.setDescription("Hello World!");
...
//		// ignore
//		System.out.println("PhabricatorTaskDataHandler.getMultiTaskData()");
//
...
//	}
	...
	public boolean initializeSubTaskData(TaskRepository repository, TaskData taskData, TaskData parentTaskData,
		...
		System.err.println("TracTaskDataHandler.initializeSubTaskData()");
		...
		if (blockingAttribute == null) {
			throw new CoreException(new RepositoryStatus(repository, IStatus.ERROR, PhabricatorCorePlugin.ID_PLUGIN,
	...
	public boolean canInitializeSubTaskData(TaskRepository taskRepository, ITask task) {
		System.out.println("TracTaskDataHandler.canInitializeSubTaskData()");

		System.out.println(task.getTaskId());

		return true;

//		return Boolean.parseBoolean(task.getAttribute(PhabricatorRepositoryConnector.TASK_KEY_SUPPORTS_SUBTASKS));
	}
...
//	}
	...
	public TaskAttributeMapper getAttributeMapper(TaskRepository repository) {
		System.out.println("TracTaskDataHandler.getAttributeMapper()");
//		ITracClient client = connector.getClientManager().getTracClient(repository);
		return new PhabricatorAttributeMapper(repository);
	}
...
//				PhabricatorRepositoryConnector.getTicketId(data.getTaskId()));
//
//		Collection<TaskAttribute> attributes = data.getRoot().getAttributes().values();
//		for (TaskAttribute attribute : attributes) {
//			if (PhabricatorAttributeMapper.isInternalAttribute(attribute)
...
//		List<String> removeValues = PhabricatorRepositoryConnector.getAttributeValues(data, PhabricatorAttributeMapper.REMOVE_CC);
//		List<String> values = PhabricatorRepositoryConnector.getAttributeValues(data, TaskAttribute.USER_CC);
...
//			}
//		}
//		if (PhabricatorRepositoryConnector.getAttributeValue(data, PhabricatorAttributeMapper.NEW_CC).length() > 0) {
//			if (sb.length() > 0) {
//				sb.append(","); //$NON-NLS-1$
//			}
//			sb.append(PhabricatorRepositoryConnector.getAttributeValue(data, PhabricatorAttributeMapper.NEW_CC));
//		}
//		if (Boolean.TRUE.equals(PhabricatorRepositoryConnector.getAttributeValue(data, TaskAttribute.ADD_SELF_CC))) {
//			if (sb.length() > 0) {
//				sb.append(","); //$NON-NLS-1$
//			}
...
//
//		ticket.putValue("owner", PhabricatorRepositoryConnector.getAttributeValue(data, TaskAttribute.USER_ASSIGNED)); //$NON-NLS-1$
//
...
//					ticket.putValue("status", PhabricatorRepositoryConnector.TaskStatus.ASSIGNED.toStatusString()); //$NON-NLS-1$
//				} else if ("resolve".equals(action)) { //$NON-NLS-1$
//					ticket.putValue("status", PhabricatorRepositoryConnector.TaskStatus.CLOSED.toStatusString()); //$NON-NLS-1$
//					ticket.putValue("resolution", PhabricatorRepositoryConnector.getAttributeValue(data, //$NON-NLS-1$
//							TaskAttribute.RESOLUTION));
//				} else if ("reopen".equals(action)) { //$NON-NLS-1$
//					ticket.putValue("status", PhabricatorRepositoryConnector.TaskStatus.REOPENED.toStatusString()); //$NON-NLS-1$
//					ticket.putValue("resolution", ""); //$NON-NLS-1$ //$NON-NLS-2$
//				} else if ("reassign".equals(action)) { //$NON-NLS-1$
//					ticket.putValue("status", PhabricatorRepositoryConnector.TaskStatus.NEW.toStatusString()); //$NON-NLS-1$
//				}
//			}
//			ticket.putValue("action", action); //$NON-NLS-1$
//		}
//
//		Date lastChanged = TracUtil.parseDate(PhabricatorRepositoryConnector.getAttributeValue(data,
...
//	}
	...
	public void migrateTaskData(TaskRepository taskRepository, TaskData taskData) {
		System.out.println("TracTaskDataHandler.migrateTaskData()" + taskData.getTaskId());

//		TaskAttribute root = taskData.getRoot();
//
//		TaskAttribute attribute = root.getAttribute(TaskAttribute.USER_ASSIGNED);

//		root.createAttribute("Zil");
//		root.getMetaData().setType("CAN");
//		root.getMetaData().setLabel("SFDSFD");
//		root.setValue("Zlll");
...
//				} else if (PhabricatorAttributeMapper.NEW_CC.equals(attribute.getId())) {
...
//				}
//			}
//			if (root.getAttribute(PhabricatorAttributeMapper.REMOVE_CC) == null) {
//				root.createAttribute(PhabricatorAttributeMapper.REMOVE_CC);
//			}
